...
(function () {
  ...
  function saveSettings() {
    window.localStorage.setItem(
      SETTINGS_KEY,
      JSON.stringify({ autoSend: state.autoSend }),
    );
  }
  ...
  function getIcon(name) {
    const icons = {
      search:
        '<svg viewBox="0 0 24 24" fill="none"><circle cx="11" cy="11" r="6.5" stroke="currentColor" stroke-width="1.8"></circle><path d="M16 16L20 20" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"></path></svg>',
      ...
      close:
        '<svg viewBox="0 0 24 24" fill="none"><path d="M7 7L17 17M17 7L7 17" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"></path></svg>',
  ...
  function appendButton() {
    ...
    const moreButton = document.querySelector(
      'button[data-test-id="skill_bar_button_more"]',
    );
    if (!moreButton) return;
    ensureUi();
    const anchor =
      moreButton.parentElement?.tagName === "DIV"
        ? moreButton.parentElement
        : moreButton;
    anchor.insertAdjacentElement(
      "afterend",
      createButton(moreButton.className),
    );
  }
  ...
  function getFilteredPrompts() {
    ...
    return state.prompts.filter((item) =>
      `${item.name} ${item.text}`.toLowerCase().includes(keyword),
    );
  }
  ...
  function bindPanelEvents() {
    ...
    if (addNameInput)
      addNameInput.addEventListener("input", (event) => {
        state.addNameDraft = event.target.value;
      });
    const addTextarea = panel.querySelector("#tm-add-textarea");
    if (addTextarea)
      addTextarea.addEventListener("input", (event) => {
    ...
    if (editNameInput)
      editNameInput.addEventListener("input", (event) => {
        state.editNameDraft = event.target.value;
      });
    ...
    panel
      ...
      .forEach((element) =>
        element.addEventListener("click", handlePanelAction),
      );
    panel
      .querySelectorAll(".tm-prompt-card[draggable='true']")
      .forEach((element) => {
  ...
  function clearDropTargets() {
    document
      .querySelectorAll(".tm-prompt-card.is-drop-target")
      .forEach((element) => {
  ...
  function handleDrop(event) {
    ...
    const draggingId =
      state.draggingId || event.dataTransfer?.getData("text/plain") || "";
    clearDropTargets();
  ...
  function handleDragEnd() {
    ...
    clearDropTargets();
    document
      .querySelectorAll(".tm-prompt-card.is-dragging")
      .forEach((element) => {
  ...
  function isInCustomUi(element) {
    return Boolean(
      ...
      element?.closest?.(`#${CUSTOM_BUTTON_ID}`),
    );
  }
  ...
  function scoreComposerCandidate(element) {
    ...
    if (
      hintText.includes("发送") ||
      hintText.includes("输入") ||
      hintText.includes("message")
    )
      score += 24;
  ...
  function scoreSendButton(button, composer) {
    if (!isVisible(button) || isInCustomUi(button) || button.disabled)
      return -Infinity;
    const id = button.id || "";
    const dataDbxName = button.getAttribute("data-dbx-name") || "";
    const text = [
      button.textContent || "",
    ...
    let score = 0;
    // 豆包特定的发送按钮ID识别
    if (id === "flow-end-msg-send") score += 100;
    // 通用发送按钮文本识别
    if (text.includes("发送") || text.includes("send")) score += 48;
    if (text.includes("submit")) score += 20;
    // 发送按钮类名识别
    if (text.includes("send-btn") || text.includes("send_btn")) score += 24;
    if (
      button.closest("form") &&
      composer.closest("form") === button.closest("form")
    )
      score += 24;
  ...
  function triggerSend(composer) {
    ...
    if (sendButton) {
      ...
      if (sendButton.disabled || !isVisible(sendButton)) {
        console.warn("[tm-prompt-panel] 发送按钮已禁用或不可见");
        return false;
      }
      // 先尝试普通点击
      sendButton.click();
      // 如果Click后状态未改变，尝试其他方法
      return true;
    }
    composer.focus();
    const options = {
